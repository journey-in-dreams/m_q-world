...
type ControlledTweakpaneProps = {
	binds: Binds;
	onBindsChange?: (binds: Binds) => void;
};

type UncontrolledTweakpaneProps = {
	initialBinds: Binds;
	onBindsChange?: (binds: Binds) => void;
};
...
const renderNumber = (
	...
	return 'min' in bind && 'max' in bind ? (
		<div key={key} className="flex flex-row gap-2.5">
			<div className="w-20 flex items-center shrink-0 min-w-0">
				<Label
		...
		<div key={key} className="flex flex-row gap-2.5">
			<div className="w-20 truncate flex items-center shrink-0 min-w-0">
				<Label
					className="truncate text-current/80 block leading-[1.2]"
					htmlFor={key}
				>
